 ...
 *
 * Какие документы обязательны, решает вакансия: у разработчика это паспорт,
 * СНИЛС и ИНН, у того, кто работает с деньгами клиентов, добавляется проверка
 * службой безопасности. Статус считается из даты триггером в базе и не
 * редактируется руками — иначе он врёт.
 */
  ...
  return (
    <>
      <PageHeader
        ...
        title="Документы"
        description="Сначала то, что горит: просроченное и истекающее в ближайший месяц. Пока обязательные для вакансии документы не закрыты, отклик нельзя двинуть на оффер."
      />
      ...
      {docs.loading ? (
        ...
        <EmptyState
          ...
          description="Кандидаты присылают документы прямо в чат бота — здесь появится чек-лист по каждому."
        />
      ) : (
        <>
          {problems.length > 0 && (
            <Card className="mb-4 border-l-[3px] border-l-warn">
              <p className="m-0 text-[13.5px] text-ink-2">
                ...
                Оформление занимает недели — запрашивать нужно заранее,
                а не в день выхода.
              </p>
            </Card>
          )}
